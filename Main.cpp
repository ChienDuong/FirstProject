...
void findHomography(vector<vector<double> > src, vector<vector<double> > dst, float homography[9])
{
	float P[8][9] = {
	...
	float aux_H[] = { P[0][8], P[1][8], P[2][8], // h11  h12 0 h13  
		P[3][8], P[4][8], P[5][8], // h21  h22  h23		      
		P[6][8], P[7][8], 1 };      // h31  h32 0 h33  
...
int main()
{
	...
	tic = clock();
	cout << "tic:" << tic << endl;
	vector<vector<double> > SCAN(4, vector<double>(2));
	...
	PointsForWarp.close();
	// CAD points
	vector<vector<double> > CAD(4, vector<double>(2));
	...
	for (int indexX_2 = 0; indexX_2 < Hor_dataXY.size(); indexX_2 = (indexX_2 + Hor_dataXY.size() - 1))   // !!X_2 HOr = 105x3138 , X_1 Ver =105x3111	
	{
		...
		int indexX_1 = 0;
		for (int indexY_2 = 0; indexY_2 < Hor_dataXY[0].size(); indexY_2++)// cua horizoltal 3138, xet tat ca cac diem cua current horizoltal
		{
			for (int indexY_1 = 0; indexY_1 < Ver_dataXY[0].size(); indexY_1++)// cua vertical 3111
			{
		...
		for (int indexY_2 = 0; indexY_2 < Hor_dataXY[0].size(); indexY_2++)// cua horizoltal 3138, xet tat ca cac diem cua current horizoltal, Y_2: horizoltal, Y_1, vertical
		{
			for (int indexY_1 = firstY_1 - 2; indexY_1 < firstY_1 + 2; indexY_1++)// bay h chi xet 5 diem dao dong xung quanh, diem dau tien cross
			{
				...
				if (indexY_1<0) // check the cross is the first point or not!
				{
	...
	//4***********************************Plot Vertical boundary left nd right *********************************************************************************//
	...
	for (int indexX_2 = 0; indexX_2 < Ver_dataXY.size(); indexX_2 = (indexX_2 + Ver_dataXY.size() - 1))   // !! HOr = 105x3138 , Ver =105x3111
	{
		...
		for (int indexY_2 = 0; indexY_2 < Ver_dataXY[0].size(); indexY_2++)// cua horizoltal 3138, xet tat ca cac diem cua current horizoltal
		{
			...
			{
				if (indexX_1>Hor_dataXY.size() )//!! Chu y do khac nhau giua matLab va C nen phai tru 1, ngoai line 105 bo?
				{
					break;
				}
				if (indexX_1 == 0)
...
//4***********************************Finished Plot Vertical boundary left and right ********************************************************************************//
	...
	if (switch_V_H == 1 || switch_V_H == 2)
	{
		...
		vector<vector<double> > HorXY_Homo(Hor_XY.size(), vector<double>(2));
		for (int i = 0; i < Hor_XY.size(); i++)
		{
			HorXY_Homo[i][0] = (H[0] * Hor_XY[i][0] + H[1] * Hor_XY[i][1] + H[2]) / (H[6] * Hor_XY[i][0] + H[7] * Hor_XY[i][1] + 1);
			HorXY_Homo[i][1] = (H[3] * Hor_XY[i][0] + H[4] * Hor_XY[i][1] + H[5]) / (H[6] * Hor_XY[i][0] + H[7] * Hor_XY[i][1] + 1);
		}
		/*toc = clock();
		...
		vector<vector<double> > XYT_hom(Hor_XY.size(), vector<double>(2));
		 XYT_hom = HorXY_Homo;
		...
		//for (int i = cross_point_LandR.size() / 2 +1; i < cross_point_LandR.size(); i++)
		for (int i = cross_point_LandR.size() / 2 ; i < cross_point_LandR.size(); i++)
		{
			//bound_right[i - (cross_point_LandR.size() / 2+1 )] = cross_point_LandR[i];//177,24
		...
		/*	writerXY << "zoom" << endl;
			...
			writerXY << "Pline" << endl;
			int temp = 0;
			for (int i = 0; i < generate_data.size(); i++)//0->104
			{
				
				//if (i==104||i==103)
				//{
				//	//cout << "gia tri in ra:" << XYT_hom[generate_data[i][j]][0];
				//	cout << "gia tri boundleft:" << bound_left[i][0];
				//	cout << "gia tri boundright:" << bound_right[i][1];
				//	int c = 1;
				//}
				for (int j = 0; j < generate_data[0].size(); j++)//0-3129
				{
					if (generate_data[i][j] == 0&j>0)// truong hop dau tien j=0, goc toa do dau tien =0, cac truong hop con lai phat hien diem 0 thi dung lai, vi thuc ra xet boundary minh chi xai 1 phan trong do
					{
						break;
						
					}
					else if (Hor_XY[generate_data[i][j]][0]>bound_left[i][0] && Hor_XY[generate_data[i][j]][0]<bound_right[i][0])// toa do luu theo kieu [219  , 174] [219,175], luu diem theo y truoc x sau. Xet diem truoc homo ko phai diem homo
					{

						writerXY << XYT_hom[generate_data[i][j]][0] << "," << XYT_hom[generate_data[i][j]][1] << endl;
	...
	if (switch_V_H == 0 || switch_V_H == 2)
	{
		...
		vector<vector<double> > VerXY_Homo(Ver_XY.size(), vector<double>(2));
		for (int i = 0; i < Ver_XY.size(); i++)
		{
			VerXY_Homo[i][0] = (H[0] * Ver_XY[i][1] + H[1] * Ver_XY[i][0] + H[2]) / (H[6] * Ver_XY[i][1] + H[7] * Ver_XY[i][0] + 1);
			VerXY_Homo[i][1] = (H[3] * Ver_XY[i][1] + H[4] * Ver_XY[i][0] + H[5]) / (H[6] * Ver_XY[i][1] + H[7] * Ver_XY[i][0] + 1);
		}
		/*toc = clock();
		printf("Homography: %f seconds\n", (double)(toc - tic) / CLOCKS_PER_SEC);*/
		toc = clock();
		...
		vector<vector<double> > XYT_hom(Ver_XY.size(), vector<double>(2));
		XYT_hom = VerXY_Homo;
		...
		if (writerXY.is_open()) // buoc nay khi viet code rat quan trong, vi can phai kiem tra ghi dc hay o
		{
			...
			for (int i = 0; i < generate_data.size(); i++)
			{
				...
				for (int j = 0; j < generate_data[0].size(); j++)
				{
					...
					else if (Ver_XY[generate_data[i][j]][0]>bound_down[i][1] && Ver_XY[generate_data[i][j]][0]<bound_up[i][1])
					{
						writerXY << XYT_hom[generate_data[i][j]][0] << "," << XYT_hom[generate_data[i][j]][1] << endl;
					}
					else
